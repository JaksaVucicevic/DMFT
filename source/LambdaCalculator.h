...
class LambdaCalculator
{
  private:
    ...
    //----------options----------------//
    int returned_Nfreq;				// parameters for lambda that is the return value of CalculateLambda method. Number of freqs used and
    bool returned_FromContinued;				// whether input or continued X is used
  ...
  public:
    ...
    void SetLambdaReturned(int returned_Nfreq, bool returned_FromContinued=false);
    void SetOutputFileName(const char* FN);	//only the parameters extnesion (e.g. ".U2.500.T0.050.W1.000.n0.500"). the part "diffs", "lambdas" or "lambdas.continued." is automaticcaly inserted at the beginning
